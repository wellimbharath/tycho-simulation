...
use super::traces::{handle_traces, TraceResult};
use ethers::types::{Bytes, U256};

use foundry_config::{Chain, Config};
use foundry_evm::inspectors::InspectorStackBuilder;
use foundry_evm::traces::{TraceKind, TraceMode};
use revm::interpreter::{return_ok, InstructionResult};
use revm::primitives::{EVMResultGeneric, ResultAndState};
use revm::{
    db::DatabaseRef,
    ...
    D::Error: std::fmt::Debug,
{
    ...
    fn print_traces(mut tracer: TracingInspector, res: &ResultAndState) {
        if let ResultAndState { result, state: state_changeset } = res {
            let (exit_reason, _gas_refunded, gas_used, _out, _exec_logs) = match result.clone() {
                ExecutionResult::Success {
                    reason, gas_used, gas_refunded, output, logs, ..
