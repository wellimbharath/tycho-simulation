...
pub struct SimulationDB<M: Middleware + Clone> {
    db: CacheDB<EthRpcDB<M>>,
...
impl<M: Middleware + Clone> SimulationDB<M> {
    ...
    fn track_miss(&mut self, address: B160) {
        if !self.db.accounts.contains_key(&address) {
    ...
    pub fn update_state(
        ...
        self.db.db.block = Some(BlockId::Number(BlockNumber::Number(block.number.into())));
...
impl<M: Middleware + Clone> Database for SimulationDB<M> {
    type Error = M::Error;
    ...
    fn storage(&mut self, address: B160, index: rU256) -> Result<rU256, Self::Error> {
        ...
        if self.mocked_accounts.contains(&address) {
            if let Some(db_account) = self.db.accounts.get(&address) {
