...
// }
//
// impl<D: DatabaseRef + std::fmt::Debug + 'static + std::marker::Send> ProtocolSim
//     for DodoPoolState<D>
// where
//     D::Error: std::fmt::Debug,
// {
...
//     > { let calldata = if self.base_token == token_in.address { self.helper_abi
//     > .encode("querySellBaseToken", (self.pool_address, amount_in)) } else { self.helper_abi
//     > .encode("querySellQuoteToken", (self.pool_address, amount_in)) } .expect("DODO: Error
//     > encoding calldata for get_amount_out!"); let params = SimulationParameters { caller:
//     > Address::ZERO, to: Address::from(self.helper_address.0), data: calldata, value:
//     > U256::zero(), overrides: None, gas_limit: None, block_number: 0, timestamp: 0, }; let
//     > engine = self.engine.borrow(); let simulation_result = engine.simulate(&params).unwrap();
//     > let amount_out = self .pool_abi .decode_output::<U256, _>("querySellBaseToken",
//     > simulation_result.result) .expect("DODO: Failed decoding get_amount_out result!");
