...
use crate::safe_math::{safe_div, safe_mul};
...
pub fn get_sqrt_ratio_at_tick(tick: i32) -> U256 {
    ...
    if abs_tick.bit(1) {
        ratio = (safe_mul(ratio , U256([6459403834229662010, 18444899583751176498, 0, 0]))) >> 128
    }
    if abs_tick.bit(2) {
        ratio = (safe_mul(ratio , U256([17226890335427755468, 18443055278223354162, 0, 0]))) >> 128
    }
    if abs_tick.bit(3) {
        ratio = (safe_mul(ratio, U256([2032852871939366096, 18439367220385604838, 0, 0]))) >> 128
    }
    if abs_tick.bit(4) {
        ratio = (safe_mul(ratio, U256([14545316742740207172, 18431993317065449817, 0, 0]))) >> 128
    }
    if abs_tick.bit(5) {
        ratio = (safe_mul(ratio ,U256([5129152022828963008, 18417254355718160513, 0, 0]))) >> 128
    }
    if abs_tick.bit(6) {
        ratio = (safe_mul(ratio, U256([4894419605888772193, 18387811781193591352, 0, 0]))) >> 128
    }
    if abs_tick.bit(7) {
        ratio = (safe_mul(ratio, U256([1280255884321894483, 18329067761203520168, 0, 0]))) >> 128
    }
    if abs_tick.bit(8) {
        ratio = (safe_mul(ratio, U256([15924666964335305636, 18212142134806087854, 0, 0]))) >> 128
    }
    if abs_tick.bit(9) {
        ratio = (safe_mul(ratio, U256([8010504389359918676, 17980523815641551639, 0, 0]))) >> 128
    }
    if abs_tick.bit(10) {
        ratio = (safe_mul(ratio, U256([10668036004952895731, 17526086738831147013, 0, 0]))) >> 128
    }
    if abs_tick.bit(11) {
        ratio = (safe_mul(ratio, U256([4878133418470705625, 16651378430235024244, 0, 0]))) >> 128
    }
    if abs_tick.bit(12) {
        ratio = (safe_mul(ratio, U256([9537173718739605541, 15030750278693429944, 0, 0]))) >> 128
    }
    if abs_tick.bit(13) {
        ratio = (safe_mul(ratio, U256([9972618978014552549, 12247334978882834399, 0, 0]))) >> 128
    }
    if abs_tick.bit(14) {
        ratio = (safe_mul(ratio, U256([10428997489610666743, 8131365268884726200, 0, 0]))) >> 128
    }
    if abs_tick.bit(15) {
        ratio = (safe_mul(ratio, U256([9305304367709015974, 3584323654723342297, 0, 0]))) >> 128
    }
    if abs_tick.bit(16) {
        ratio = (safe_mul(ratio, U256([14301143598189091785, 696457651847595233, 0, 0]))) >> 128
    }
    if abs_tick.bit(17) {
        ratio = (safe_mul(ratio , U256([7393154844743099908, 26294789957452057, 0, 0]))) >> 128
    }
    if abs_tick.bit(18) {
        ratio = (safe_mul(ratio, U256([2209338891292245656, 37481735321082, 0, 0]))) >> 128
    }
    if abs_tick.bit(19) {
        ratio = (safe_mul(rati, U256([10518117631919034274, 76158723, 0, 0]))) >> 128
    }

    if tick > 0 {
        ratio = safe_div(U256::MAX, ratio)?;
    }
